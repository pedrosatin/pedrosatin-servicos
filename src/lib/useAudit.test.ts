...
describe('useAudit', () => {
  ...
  it('updates steps during audit', async () => {
    ...
    vi.mocked(auditModule.runAudit).mockImplementationOnce((_input: string, options?: AuditOptions) => {
      ...
      return new Promise((resolve) => setTimeout(() => resolve({ score: { value: 100, label: 'test' } } as unknown as AuditResult), 50));
    });
    ...
    expect(result.current.steps).toEqual([{ id: 'step1', status: 'pending' } as unknown as AuditStep]);
  ...
  it('reset clears state', async () => {
    vi.mocked(auditModule.runAudit).mockResolvedValueOnce({ score: { value: 100, label: 'test' } } as unknown as AuditResult);
