...
describe('useAudit', () => {
  ...
  it('updates steps during audit', async () => {
    ...
    vi.mocked(auditModule.runAudit).mockImplementationOnce((_input: string, options: AuditOptions = {}) => {
      ...
      return new Promise((resolve) => setTimeout(() => resolve({ score: { value: 100, label: 'good' } } as unknown as AuditResult), 50));
    });
    ...
    act(() => {
      if (onStepCallback) onStepCallback({ id: 'step1', status: 'running' } as AuditStep);
    });

    expect(result.current.steps).toEqual([{ id: 'step1', status: 'running' }]);

    act(() => {
      if (onStepCallback) onStepCallback({ id: 'step2', status: 'pending' } as AuditStep);
    });
    ...
    act(() => {
      if (onStepCallback) onStepCallback({ id: 'step3', status: 'pending' } as AuditStep);
    });
  ...
  it('reset clears state', async () => {
    vi.mocked(auditModule.runAudit).mockResolvedValueOnce({ score: { value: 100, label: 'good' } } as unknown as AuditResult);
